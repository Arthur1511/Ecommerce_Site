...
def index(request):
    ...
    if user:
        ...
        items = Item.objects.filter(cart__user=user, cart__check_out=False)
...
def cart(request):
    ...
    if user:
        ...
        cart_items = Item.objects.filter(cart__user=user, cart__check_out=False)
...
def confirm_order(request):
    user = request.user
    cart_items = Item.objects.filter(cart__user=user, cart__check_out=False)
    item_sum = calculate_sum(cart_items)
    cart = Cart.objects.get(user=user, check_out=False)
    cart.check_out = True
    cart.save()
    User.objects.filter(username=user.username).delete()
    return render(request, 'thankyou.html',
                  {'user': user, 'items': 0, 'page': 'cart', 'cart_items': cart_items, 'sum': item_sum,
